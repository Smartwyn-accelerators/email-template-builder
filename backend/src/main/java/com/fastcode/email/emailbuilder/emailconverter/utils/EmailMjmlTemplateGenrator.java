...
import com.fastcode.email.emailbuilder.emailconverter.dto.request.Height;
import org.apache.commons.io.IOUtils;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.fastcode.email.emailbuilder.emailconverter.dto.request.Background;
import com.fastcode.email.emailbuilder.emailconverter.dto.request.Border;
import com.fastcode.email.emailbuilder.emailconverter.dto.request.Elements;
import com.fastcode.email.emailbuilder.emailconverter.dto.request.LineHeight;
import com.fastcode.email.emailbuilder.emailconverter.dto.request.Options;
import com.fastcode.email.emailbuilder.emailconverter.dto.request.Padding;
import com.fastcode.email.emailbuilder.emailconverter.dto.request.Request;
import com.fastcode.email.emailbuilder.emailconverter.dto.request.Size;
import com.fastcode.email.emailbuilder.emailconverter.dto.request.Structures;
import com.fastcode.email.emailbuilder.emailconverter.dto.request.Width;
@Service
public class EmailMjmlTemplateGenrator {
	...
	private String createPadding(Padding padding) {
		return CommonUtil.checkValue.apply(padding.getTop(), "10") + "px " + CommonUtil.checkValue.apply(padding.getRight(), "25") + "px "
				+ CommonUtil.checkValue.apply(padding.getBottom(), "10") + "px " + CommonUtil.checkValue.apply(padding.getLeft(), "25")
				+ "px";
	}

	private String createBackground(Background background) {
		return CommonUtil.checkValue.apply(background.getColor(), "white") + " " + CommonUtil.checkValue.apply(background.getUrl(), "")
				+ " && " + CommonUtil.checkValue.apply(background.getUrl(), "") + " "
				+ CommonUtil.checkValue.apply(background.getRepeat(), "no-repeat") + " top center";
	}
	...
	private String createBorder(Border border) {
		return CommonUtil.checkValue.apply(border.getWidth(), "4") + "px " + CommonUtil.checkValue.apply(border.getStyle(), "solid") + " "
				+ CommonUtil.checkValue.apply(border.getColor(), "#000000");
	}

	private String createLineHeight(LineHeight lineHeight) {
		return lineHeight.getUnit().compareToIgnoreCase("none") != 0
				? CommonUtil.checkValue.apply(lineHeight.getValue(), "22") + CommonUtil.checkValue.apply(lineHeight.getUnit(), "px") : "120%";
	}
	...
	private String genrateTemplateSection(Structures structures) throws IOException {
		String template = this.text2String(env.getMjmlFileBaseDir() + "/sectionTemp.txt");
	...
	public String genrateTemplateBody() throws IOException {
		System.out.println(env.getMjmlFileBaseDir() + "/basicTemp.txt");
		String template = this.text2String(env.getMjmlFileBaseDir() + "/basicTemp.txt");
