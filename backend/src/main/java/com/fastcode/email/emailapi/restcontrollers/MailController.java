package com.fastcode.email.emailapi.restcontrollers;
...
import com.fastcode.email.emailapi.application.mail.dto.CreateEmailInput;
import com.fastcode.email.emailapi.application.mail.EmailService;
...
//	@PreAuthorize("hasAnyAuthority('SENDEMAIL')")
	...
	public ResponseEntity sendEmail(@RequestBody @Valid CreateEmailInput email) throws IOException {
		...
		if(mergeFieldMapped!=null && mergeFieldMapped.size()>0)
		{
			...
			Query<Object> managerQuery = session.createNativeQuery(query);
	...
	private String replaceVariable(String input,HashMap<String,String> mergeFieldData, HashMap<Long, byte[]> imageDataSourceMap) {
		...
		Pageable pageable = new OffsetBasedPageRequest(Integer.parseInt(env.getFastCodeOffsetDefault()),
				Integer.parseInt(env.getFastCodeLimitDefault()));
				...
				case "Image":
					if(isReplaced) {
						...
						input = input.replaceAll(Pattern.quote(m.group(0)), getImage(fileIdPrevious));
						 ...
						 Optional<File> f = filesRepo.findById(Long.parseLong(fileIdPrevious));
							if (f.isPresent()) {
